/* 
 * Compile with nvcc -gencode arch=compute_30,code=sm_30 gatherSearch.cu 
 */
...
// Utility function for string parsing from K&R
...
int search(char * keys, int keys_length, int key_count, int data_length) {
   ...
   // Allocate memory on the device and copy the keys
   ...
   for(int i = 0; data_length - BYTES_TO_SEARCH * i > 0; ++i) {
      ...
      errorChecking(cudaMemcpy(data_dev0, (data+BYTES_TO_SEARCH*i), 
         segment_length, cudaMemcpyHostToDevice), __LINE__);

/* 
 * Compile with: 
 * nvcc -gencode arch=compute_30,code=sm_30 scatterSubstringSearch1.cu 
 */
...
// Set up the device and call the kernel
int search(char *pattern, int pattern_length, char *string, int string_length) {
   ...
   errorChecking(cudaMalloc((void **) &pattern_dev, 
      sizeof(char)*pattern_length), __LINE__);

   // Copy the count to the device
   errorChecking( cudaMemcpyToSymbol(count_dev, &count, sizeof(int), 0, 
      cudaMemcpyHostToDevice), __LINE__);

   // Copy pattern and data set to the device
   for(int i = 0; i < numStreams; ++i) {
...
// Grab patterns from the file specified in main and write the results 
// to a file
int count_keys_in_file(char *filename, char * string, int string_length) {
   ...
   while(fgets(BUFFER, BUFF_SIZE, infile) != NULL) {
      ...
      // Call the kernel load function and get count

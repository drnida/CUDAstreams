/* 
 * Compile with 
 * nvcc -gencode arch=compute_30,code=sm_30 scatterSubstringSearch2.cu
 */
...
int search(char *filename, char * string_dev, int string_length, 
      cudaStream_t * stream) {
   FILE *infile;
   FILE *outfile;
   ...
   // Grab each pattern from the file
   while(!feof(infile)) {
      for(int i=0; i<numStreams && fgets(BUFFER[i], BUFF_SIZE, infile) != NULL;
            ++i) {
         j = i + 1;
         ...
         // Load count and pattern onto device 
         errorChecking(cudaMemcpyToSymbolAsync(count_dev, &count, sizeof(int), 
            sizeof(int)*i, cudaMemcpyHostToDevice, stream[i]), __LINE__);
         errorChecking(cudaMemcpyAsync(&pattern_dev[i*1024], 
            BUFFER[i], length[i]*sizeof(char), cudaMemcpyHostToDevice, 
            stream[i]), __LINE__);
      }
...
int main(void) {
   ...
   errorChecking( cudaMemcpyAsync(&string_dev[0], 
      &string[0],  string_length * sizeof(char), cudaMemcpyHostToDevice, 
      stream[0] ), __LINE__);
   cudaStreamSynchronize(stream[0]); 

   // Call the kernel launch function
   gettimeofday(&start, 0); 
   search("key_list.txt", string, string_length, stream);
   gettimeofday(&end, 0); 
